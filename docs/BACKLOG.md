...
- [ ] **`insights.py` does not remap legacy parquet columns**: `dataset_data.py` reconciles SDMX ↔ `*_nom_id` names via `sdmx_column_map`; `insights.py` builds its own slice and does not, so every legacy parquet logs `Binder Error: Referenced column "TIME_PERIOD" not found` and falls back to a single KPI. POP201D shows 1 KPI where it should show 5. Pre-existing, confirmed against the pre-2026-09-01 code.
