...
- [ ] **🔴 `12-parquet-to-sdmx.py` overwrites good data with a lossy February snapshot — 1,108 canonical parquets affected** (diagnosed 2026-09-04). Two scripts write to `data/corpus/parquet/` and one of them reads a dead input:
    - `9-csv-to-parquet.py` reads `data/4-datasets/ro/*.csv` (original labels) and writes `corpus/parquet/{code}.parquet` with `*_nom_id` column names holding **text labels** — this is what the app calls a "legacy" parquet, and it is **complete and current**. (`duckdb_config.PARQUET_DIR = CORPUS_PARQUET_DIR`.)
    - `12-parquet-to-sdmx.py` reads `data/parquet-v2/{lang}/` — 98% of it a Feb–Apr 2026 snapshot produced by `7-data-compactor.py`, which has been **commented out of `update-pipeline.py`** — and writes to the *same* output path, replacing stage 9's file.
    - The compactor matched CSV labels against metadata labels literally. INS emits comma-delimited CSVs and rewrites commas inside values as double spaces, so **every label containing a comma failed to match and was stored as NULL**. TUR104C: `Zona Delta Dunarii, inclusiv orasul Tulcea` → missing; 3 of 6 destinations gone, 188 of 380 rows NULL. TRN113A: 5 of 7 wagon types gone, all five comma-bearing.
    - Because `query_builder` adds `"col" IS NOT NULL` to every grouped query, those rows vanish from charts, headlines and rankings with no indication.
    - **Scale**: 1,108 of 3,769 canonical parquets carry NULL dimension values (3.6M of 87.5M rows; 51 files where *every* row does). Of the 482 that have an original CSV to compare against, a random sample of 60 was **60/60 demonstrably losing dimension values**.
    - The data is not lost — the original CSVs are intact, and re-running `9-csv-to-parquet.py` on TRN113A reproduced all 7 wagon types exactly. **Needs a design decision**, so not attempted: (a) point stage 12 at stage 9's label output and map labels→SDMX values, (b) regenerate `parquet-v2` with comma-tolerant matching and re-convert, or (c) retire stage 12 and have stage 9 emit SDMX directly. (a) or (c) look right; (b) keeps a directory nothing else updates.
    - Until then, **do not run `12-parquet-to-sdmx.py`** on a matrix whose corpus parquet is currently stage-9 output — it will overwrite complete data with the February version.
    - **Decided 2026-09-04: option (c).** Implementation spec written: [stage9-sdmx-migration-spec.md](stage9-sdmx-migration-spec.md). Phases A–D, verification gates, and escalation rules are in that document. Not started.
